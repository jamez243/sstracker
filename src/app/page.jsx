...
function MainComponent() {
  ...
  const [previousSeasonStats, setPreviousSeasonStats] = useState(() => {
    const savedStats = localStorage.getItem("previousSeasonStats");
  ...
  const getTopScorer = () => {
    ...
    return players.reduce((max, player) =>
      max.goals > player.goals ? max : player
    );
  };
  const getTopAssister = () => {
    if (players.length === 0) return null;
    return players.reduce((max, player) =>
      max.assists > player.assists ? max : player
    );
  };
  ...
  const resetAllData = () => {
    if (window.confirm("ARE YOU SURE?")) {
      ...
      localStorage.removeItem("previousSeasonStats");
            ...
            >
              ...
              <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="p-4 bg-blue-100 rounded">
                  <h3 className="font-bold mb-2">Top Assister</h3>
                  {topAssister && (
                    <div>
